...
@Stateless
@Path("/container")
public class ContainerFacadeREST extends AbstractFacade<Container> {

    @PersistenceContext(unitName = "org.homecloud.inventory.pu")
    private EntityManager em;

    public ContainerFacadeREST() {
    ...
    public Container find(@PathParam("id") String id) {
    ...
    @GET
    @Produces({MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON})
    public List<Container> findRange(@QueryParam("page") Integer page, 
                                     @QueryParam("pageSize") Integer pageSize,
                                     @QueryParam("sort")List<String> sorts) {
        PageRequest pr = new PageRequest(page,pageSize);
        return super.findRange(new int[]{page*pageSize, to});
    }
